...
public class Bull extends Herbivores {
    ...
    public Bull(String name, int x, int y, boolean atbirth) {
        ...
        this.food_stuff_to_full_satiety = 100; //сколько нужно кг пищи чтобы животному насытиться

        //Bull.count_in_cell[this.x][this.y] =  Bull.count_in_cell[this.x][this.y] + 1;
    }
    ...
    public void eat() {
        ...
        if (how_much_grass_to_eat > 0) {
            ...
            //System.out.println("Бык " + this.hashCode() + " пощипала траву в ячейке x = " + this.getX() + ", y = " + this.getY());
    ...
    public Animal reproduce(Animal mom, Animal dad) {
        ...
        Bull foal = (Bull) Farm.Birth("Бык", mom, dad);
    ...
    public void remove_if_dead(int x, int y) {
        ...
        Bull.total_number = Bull.total_number - 1;

        Bull.count_in_cell[x][y] = Bull.count_in_cell[x][y] - 1;

        //System.out.println("Погиб Бык " + this.hashCode() + " в ячейке x = " + this.x + ", y = " + this.y);
    }
    ...
    public void move() {
        ...
        int step_for_x = Event.rnd(1, Bull.speed);
        int step_for_y = Event.rnd(1, Bull.speed);

        Bull.count_in_cell[this.x][this.y] = Bull.count_in_cell[this.x][this.y] - 1; //меняем статистику по количеству животного данного вида в ячейке острова

        this.make_shift(step_for_x, step_for_y);

        Bull.count_in_cell[this.x][this.y] = Bull.count_in_cell[this.x][this.y] + 1;

        //System.out.println("Бык "+this.toString()+"  "  + this.hashCode() + " переместилась в другую ячейку: x = " + this.x + ", y = " + this.y);
