...
public class Sheep extends Herbivores {
    ...
    public Sheep(String name, int x, int y, boolean atbirth) {
        ...
        this.food_stuff_to_full_satiety = 15; //сколько нужно кг пищи чтобы животному насытиться

        //Sheep.count_in_cell[this.x][this.y] =  Sheep.count_in_cell[this.x][this.y] + 1;
    }
    ...
    public void eat() {
        ...
        if (how_much_grass_to_eat > 0) {
            ...
            //System.out.println("Овца " + this.hashCode() + " пощипала траву в ячейке x = " + this.getX() + ", y = " + this.getY());
    ...
    public Animal reproduce(Animal mom, Animal dad) {
        ...
        Sheep foal = (Sheep) Farm.Birth("Овца", mom, dad);
    ...
    public void remove_if_dead(int x, int y) {
        ...
        Sheep.total_number = Sheep.total_number - 1;

        Sheep.count_in_cell[x][y] = Sheep.count_in_cell[x][y] - 1;

        //System.out.println("Погибла Овца " + this.hashCode() + " в ячейке x = " + this.x + ", y = " + this.y);
    }
    ...
    public void move() {
        ...
        int step_for_x = Event.rnd(1, Sheep.speed);
        int step_for_y = Event.rnd(1, Sheep.speed);


        Sheep.count_in_cell[this.x][this.y] = Sheep.count_in_cell[this.x][this.y] - 1; //меняем статистику по количеству животного данного вида в ячейке острова

        this.make_shift(step_for_x, step_for_y);

        Sheep.count_in_cell[this.x][this.y] = Sheep.count_in_cell[this.x][this.y] + 1;

        //System.out.println("Овца "+this.toString()+"  "  + this.hashCode() + " переместился в другую ячейку: x = " + this.x + ", y = " + this.y);
