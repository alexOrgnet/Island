...
public class Mice extends Herbivores {

    public String name = "Мышь";

    public boolean completed_turn; //признак завершения хода
    public static int max_count_per_cell = 500; //максимальное количество животных в ячейке
    ...
    public Mice(String name, int x, int y, boolean atbirth) {
        ...
        //Mice.count_in_cell[this.x][this.y] =  Mice.count_in_cell[this.x][this.y] + 1;
    }
    ...
    public void eat() {
        ...
        if (how_much_grass_to_eat > 0) {
            ...
            //System.out.println("Мышь " + this.hashCode() + " пощипал траву в ячейке x = " + this.getX() + ", y = " + this.getY());
    ...
    public Animal reproduce(Animal mom, Animal dad) {
        ...
        Mice foal = (Mice) Farm.Birth("Мышь", mom, dad);
    ...
    public void remove_if_dead(int x, int y) {
        ...
        Mice.total_number = Mice.total_number - 1;

        Mice.count_in_cell[x][y] = Mice.count_in_cell[x][y] - 1;

        //System.out.println("Погибла мышь " + this.hashCode() + " в ячейке x = " + this.x + ", y = " + this.y);
    }
    ...
    public void move() {
        ...
        int step_for_x = Event.rnd(1, Mice.speed);
        int step_for_y = Event.rnd(1, Mice.speed);


        Mice.count_in_cell[this.x][this.y] = Mice.count_in_cell[this.x][this.y] - 1; //меняем статистику по количеству животного данного вида в ячейке острова

        this.make_shift(step_for_x, step_for_y);

        Mice.count_in_cell[this.x][this.y] = Mice.count_in_cell[this.x][this.y] + 1;

        //System.out.println("Мышь "+this.toString()+"  "  + this.hashCode() + " переместилась в другую ячейку: x = " + this.x + ", y = " + this.y);
